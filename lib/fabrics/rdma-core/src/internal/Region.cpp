...
namespace mxl::lib::fabrics::rdma_core
{
    ...
    uint64_t Region::Location::id() const noexcept
    {
        return std::visit(
            overloaded{
                [](std::monostate) -> uint64_t { throw std::runtime_error("Region type is not set"); },
    ...
    ::fi_hmem_iface Region::Location::iface() const noexcept
    {
        return std::visit(
            overloaded{
                [](std::monostate) -> ::fi_hmem_iface { throw std::runtime_error("Region type is not set"); },
    ...
    Region::Location Region::Location::fromAPI(mxlFabricsMemoryRegionLocation loc) noexcept
    {
        switch (loc.type)
        {
            ...
            default:                          throw std::runtime_error("Invalid memory region type");
        }
    }

    std::string Region::Location::toString() const noexcept
    {
        return std::visit(overloaded{[](std::monostate) -> std::string { throw std::runtime_error("Region type is not set"); },
    ...
    RegionGroups RegionGroups::fromFlow(FlowData& flow)
    {
        ...
        {
            throw std::runtime_error("Non-discrete flows not supported for now");
        }
        ...
        for (std::size_t i = 0; i < discreteFlow.grainCount(); ++i)
        {
            ...
            {
                throw std::runtime_error("GPU memory is not currently supported in the Flow API of MXL. Edit the code below when it is supported");
            }
