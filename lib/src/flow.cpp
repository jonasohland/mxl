...
mxlStatus mxlFlowWriterGetFlowData(mxlFlowWriter writer, mxlFlowData* out_flowData)
{
    try
    {
        if (out_flowData != nullptr)
        {
            if (auto const cppWriter = to_FlowWriter(writer); cppWriter != nullptr)
            {
                auto flowInfo = cppWriter->getFlowInfo();

                if (mxlIsDiscreteDataFormat(flowInfo.common.format))
                {
                ...
                {
                    // TODO: continuous flow writer
                    // if (auto concreteWriter- = dynamic_cast<ContinuousFlowReader*>(cppWriter-); concreteWriter- != nullptr)
                    //{
                    //    *out_flowData = concreteWriter-->flowData()->toAPI();
                    //    return MXL_STATUS_OK;
                    //}
...
mxlStatus mxlFlowReaderGetFlowData(mxlFlowReader reader, mxlFlowData* out_flowData)
{
    try
    {
        if (out_flowData != nullptr)
        {
            ...
            {
                auto flowInfo = cppReader->getFlowInfo();
                ...
                {
                    // TODO: continuous flow reader
                    // if (auto concreteReader = dynamic_cast<ContinuousFlowReader*>(cppReader); concreteReader != nullptr)
                    //{
                    //    *out_flowData = concreteReader->flowData()->toAPI();
                    //    return MXL_STATUS_OK;
                    //}
